...
function createVideoCard(video) {
    ...
    // Click on Thumbnail -> Open Ads Direct Link -> Open Preview Player
    card.addEventListener('click', () => {
        // Validation check
        if(video.adsDirectLink) {
            window.open(video.adsDirectLink, '_blank'); // Ad opens in new tab
        }
...
function setupEventListeners() {
    ...
    // Ads System Logic Step 2
    // Click on Play Overlay -> Open Video Ads Direct Link -> Play actual video
    customPlayAction.addEventListener('click', () => {
        if (!activeVideoData) return;

        // Open Second Ad in new Tab
        if (activeVideoData.videoAdsDirectLink) {
            window.open(activeVideoData.videoAdsDirectLink, '_blank');
        }
